...
from PyQt5.QtWidgets import (QApplication, QWidget, QTabWidget, QVBoxLayout, QLabel, QProgressBar,
                             ...
                             QPushButton, QMenu, QAction, QMessageBox, QInputDialog, QSizePolicy)
from PyQt5.QtCore import QTimer, Qt, QThread, pyqtSignal, QPoint, QDateTime
    ...
    def run(self):
        try:
            if platform.system() == 'Linux':
                output = subprocess.check_output(['systemctl', 'list-units', '--type=service', '--no-pager', '--all'], 
                                              text=True, timeout=5)
                services = []
                for line in output.splitlines()[1:]:
                    if line.strip():
                        ...
                        if len(parts) >= 5:
                            ...
                            description = parts[4]
                            try:
                                pid_output = subprocess.check_output(['systemctl', 'show', service, '--property=MainPID'],
                                                                  text=True, timeout=2)
                                pid = pid_output.split('=')[1].strip()
                            except:
                                pid = "N/A"
                            services.append((service, status, description, pid))
...
class TaskManager(QMainWindow):
    ...
    def init_disk_tab(self):
        ...
        self.disk_table.setHorizontalHeaderLabels(["Device", "Mountpoint", "Type", "Total", "Used", "Free", "Usage %", 
    ...
    def init_network_chart(self):
        ...
        self.network_chart.addAxis(axisX, Qt.AlignBottom)
        self.network_chart.addAxis(axisY, Qt.AlignLeft)
        
        self.sent_series.attachAxis(axisX)
        self.sent_series.attachAxis(axisY)
        self.recv_series.attachAxis(axisX)
        self.recv_series.attachAxis(axisY)
    ...
    def update_services(self):
        if not self.service_updater.isRunning():
    ...
    def update_network_chart(self):
        ...
        if len(self.sent_series.pointsVector()) > 60:
            self.sent_series.removePoints(0, 1)
            self.recv_series.removePoints(0, 1)
        
        self.sent_series.append(now.toMSecsSinceEpoch(), sent)
        self.recv_series.append(now.toMSecsSinceEpoch(), recv)
        
        self.network_chart.axisX().setRange(
            QDateTime.fromMSecsSinceEpoch((now - timedelta(seconds=60)).timestamp() * 1000),
            QDateTime.fromMSecsSinceEpoch(now.timestamp() * 1000)
        )
