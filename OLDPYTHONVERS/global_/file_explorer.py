...
class FileExplorer(QMainWindow):
    def __init__(self):
        ...
        self.set_theme(theme_manager_singleton.get_theme())
    ...
    def set_theme(self, theme_data):
        ...
        self.setStyleSheet(f"""
            QMainWindow {{ background-color: {palette['window']}; color: {palette['window_text']}; }}
            QTreeView {{ background-color: {editor['background']}; alternate-background-color: {palette['alternate_base']}; color: {editor['foreground']}; selection-background-color: {editor['selection_background']}; selection-color: {editor['selection_foreground']}; }}
            QLineEdit, QPushButton, QListWidget {{ background-color: {palette['base']}; color: {palette['text']}; border: 1px solid {palette['mid']}; border-radius: 4px; padding: 3px; }}
            QPushButton:hover {{ background-color: {palette['highlight']}; color: {palette['highlight_text']}; }}
            QPushButton:checked {{ background-color: {palette['highlight']}; color: {palette['highlight_text']}; }}
            QStatusBar {{ background-color: {palette['window']}; color: {palette['text']}; border-top: 1px solid {palette['mid']}; }}
            QLabel {{ color: {palette['window_text']}; }}
            QMenu {{ background-color: {palette['base']}; color: {palette['text']}; }}
            QMenu::item:selected {{ background-color: {palette['highlight']}; color: {palette['highlight_text']}; }}
        """)
        # Update preview pane and other custom widgets if needed
        if hasattr(self, 'preview_pane'):
            self.preview_pane.setStyleSheet(f"background: {editor['background']}; color: {editor['foreground']};")
        if hasattr(self, 'bookmark_list'):
            self.bookmark_list.setStyleSheet(f"background: {palette['base']}; color: {palette['text']};")
        if hasattr(self, 'recent_list'):
            self.recent_list.setStyleSheet(f"background: {palette['base']}; color: {palette['text']};")
        # ... add more as needed ...
