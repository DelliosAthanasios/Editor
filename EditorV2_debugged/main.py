...
# Using custom pdf_viewer.py script instead of removed pdf_viewer_widget.py
...
class MainTabWidget(QTabWidget):
    ...
    def add_pdf_tab(self, file_path):
        """Add a PDF viewer tab using the custom pdf_viewer.py script."""
        if not os.path.exists(file_path):
            return False
            
        # Launch the custom pdf_viewer.py script with the file path
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            pdf_viewer_path = os.path.join(script_dir, "pdf_viewer.py")
            
            # Launch the pdf_viewer with the specified file
            import subprocess
...
class TextEditor(QMainWindow):
    def __init__(self):
        ...
        self.music_player = None  # Will be initialized when needed
        self.init_ui()
    ...
    def open_file_in_editor_tab(self, file_path):
        ...
        # Text files and other supported formats
        try:
    ...
    def create_menu_bar(self):
        ...
        # Add Music Player to tools menu
    ...
    def apply_theme(self, theme_key):
        ...
        if theme_key in self.themes:
            ...
            # Apply editor styles to all tabs
    ...
    def set_dark_theme(self):
        """Legacy method, now uses theme manager."""
    ...
    def enable_code_explorer(self):
        """Launch the code explorer for the current tab's content."""
        current_tab = self.tabs.currentWidget()
        if not current_tab or not hasattr(current_tab, "editor"):
            QMessageBox.warning(self, "Warning", "No editor tab active.")
            return
        ...
        # Create a temporary file if the current content is not saved
        temp_file = None
        if not file_path:
            import tempfile
        ...
        try:
            ...
            # Launch the code explorer with the file
        ...
        if temp_file:
            try:
                # We don't delete immediately to allow the code explorer to read it
                QTimer.singleShot(5000, lambda: os.remove(temp_file) if os.path.exists(temp_file) else None)
            except:
                pass
                
    def open_music_player(self):
        """Open the music player widget in a new window."""
        try:
    ...
    def open_pdf_file(self):
        """Open a PDF file using the custom PDF viewer script."""
    ...
    def open_image_file(self):
        """Open an image file using the image viewer widget."""
        file_path, _ = QFileDialog.getOpenFileName(
