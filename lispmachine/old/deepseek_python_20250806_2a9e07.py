...
class LispREPLDocker:
    ...
    def send_code(self):
        ...
        try:
            ...
            # Wrap the code to ensure output is printed
            # Split into lines and process each expression
            ...
            for line in lines:
                ...
                # Check if line starts with certain keywords that don't need printing
                no_print_keywords = ['defun', 'defvar', 'defparameter', 'defmacro', 'defconstant', 
                                   'setf', 'setq', 'let', 'let*', 'flet', 'labels', 'progn',
                                   'when', 'unless', 'if', 'cond', 'case', 'loop', 'do', 'dolist',
                                   'dotimes', 'format', 'print', 'princ', 'prin1', 'write',
                                   'write-line', 'terpri']
                
                line_starts_with_keyword = any(line.lstrip('(').startswith(keyword) 
                                             for keyword in no_print_keywords)
                
                if line_starts_with_keyword or line.startswith('(format') or 'print' in line.lower():
                    # Don't wrap with print for these expressions
                    wrapped_code += line + "\n"
                else:
                    # Wrap expression with print to show the result
                    # Ensure we don't accidentally pass extra arguments to print
                    if '(' in line and line.endswith(')'):
                        # It's a function call, just wrap it
                        wrapped_code += f"(print {line})\n"
                    else:
                        # It's a simple value, print it directly
                        wrapped_code += f"(print '{line})\n"
            
            # If no expressions were found to wrap, just use the original code
            if not wrapped_code.strip():
                wrapped_code = code
            ...
            write_process.stdin.write(wrapped_code)
    ...
    def _execute_repl_command(self, command):
        ...
        try:
            ...
            # Wrap the command to ensure output is printed (unless it's already a print statement)
            if not (command.strip().startswith('(format') or 
                   'print' in command.lower() or
                   command.strip().startswith('(defun') or
                   command.strip().startswith('(setf') or
                   command.strip().startswith('(let')):
                # Handle simple values vs function calls
                if '(' in command and command.endswith(')'):
                    wrapped_command = f"(print {command})"
                else:
                    wrapped_command = f"(print '{command})"
            else:
            ...
            write_process.stdin.write(wrapped_command)
            write_process.stdin.close()
            write_process.wait()
