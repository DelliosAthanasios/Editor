...
def update_docker_compose_with_mount(container, image, mount_info):
    ...
    if container not in compose_data["services"] or compose_data["services"][container] is None:
    ...
    if "volumes" not in service or service["volumes"] is None:
        service["volumes"] = []

    # Remove any existing volume with the same mount point
    new_volumes = []
    for v in service["volumes"]:
        # v can be dict or string
        if isinstance(v, dict):
            # If it's a dict, keep it (we don't handle dict volumes here)
            new_volumes.append(v)
        elif isinstance(v, str):
            parts = v.split(":")
            if len(parts) >= 2 and parts[1] == mount_info["mount_point"]:
                continue  # skip replacing
            new_volumes.append(v)
...
def ensure_dir_in_container(container_name, target_dir):
    """Ensure the directory exists in the running container."""
    # Try to exec "mkdir -p {target_dir}" in the container
    run_subprocess(["docker", "exec", container_name, "mkdir", "-p", target_dir],
...
def mount_dialog(root, container=None, image=None):
    ...
    class MountDialog(simpledialog.Dialog):
        def body(self, master):
            ...
            # Default to /parentosfiles
            self.container_path_var = StringVar(value="/parentosfiles")
    ...
    if hasattr(dlg, "result") and dlg.result:
        ...
        # Validate input
        if not mount_info["container"] or not mount_info["host_folder"] or not mount_info["mount_point"]:
            messagebox.showerror("Input Error", "All fields must be filled.")
            return
        if not os.path.isdir(mount_info["host_folder"]):
            messagebox.showerror("Input Error", "Selected host folder does not exist or is not a directory.")
            return

        update_docker_compose_with_mount(mount_info["container"], image or "lispmachine", mount_info)
        # Stop and remove, then run with mount
